      ...
      <main className="max-w-6xl mx-auto px-6 py-10 space-y-10">
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            <section className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 relative">
              ...
              <div className="space-y-8">
                ...
                <div className="flex justify-end">
                  <button 
                    ...
                    disabled={isAnalyzing || !text || selectedEmojis.length === 0}
              ...
              <section className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2.5rem] p-10 text-white shadow-2xl animate-in fade-in zoom-in duration-500">
                <div className="flex flex-col md:flex-row gap-10 items-center">
                  <div className="flex-1">
                    ...
                    <h3 className="text-4xl font-black mb-4">Parece que sentes <span className="underline decoration-indigo-300">{lastResult.ai_analysis?.detectado}</span></h3>
                    <p className="text-lg text-indigo-100 opacity-90 leading-relaxed mb-6">
                      Intensidade: {lastResult.ai_analysis?.intensidade}/5. Lembra-te que todas as emoções são válidas.
                    </p>
                  </div>
                  
                  {lastResult.sugestao && (
                    <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-8 w-full md:w-80">
                      ...
                      <p className="font-bold text-lg mb-6 leading-snug italic">"{lastResult.sugestao.conteudo_texto}"</p>
                      {lastResult.sugestao.url_externo && (
                        <a href={lastResult.sugestao.url_externo} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-indigo-300 font-black hover:text-white transition-all">
            ...
            <section className="space-y-6">
              <h3 className="text-xl font-black flex items-center gap-2 text-slate-400">
                <Calendar size={20} /> HISTÓRICO RECENTE
              </h3>
              <div className="grid gap-4">
                {userHistory.length === 0 ? (
                  <div className="text-center py-10 bg-white rounded-3xl border-2 border-dashed border-slate-100 text-slate-300 font-medium italic">Ainda sem registos na base de dados.</div>
                ) : userHistory.map(h => (
                  <div key={h.id_registo} className="bg-white p-6 rounded-[1.5rem] flex items-center justify-between shadow-sm border border-slate-50 hover:border-indigo-100 transition-colors">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="font-black text-slate-800">{new Date(h.timestamp).toLocaleDateString('pt-PT')}</p>
                        <p className="text-slate-400 text-sm truncate max-w-[150px] md:max-w-md">{h.texto_livre}</p>
                      </div>
                    </div>
            ...
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
              ...
              <div className="space-y-4">
                {suggestions.slice(0, 5).map(s => (
                  <div key={s.id_sugestao} className="p-4 rounded-2xl hover:bg-slate-50 transition-colors group">
